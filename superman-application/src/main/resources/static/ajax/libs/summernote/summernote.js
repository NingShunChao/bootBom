...
(function (global, factory) {
  ...
  var dropdown = renderer.create('<ul class="dropdown-menu" role="list">',
      function ($node, options) {
        var markup = Array.isArray(options.items) ? options.items.map(
            function (item) {
              ...
              var dataOption = (option !== undefined) ? ' data-option="'
                  + option + '"' : '';
              return '<li role="listitem" aria-label="' + value
                  + '"><a href="#" ' + (dataValue + dataOption) + '>' + content
                  + '</a></li>';
            }).join('') : options.items;
        ...
        var markup = Array.isArray(options.items) ? options.items.map(
            function (item) {
              var value = (typeof item === 'string') ? item : (item.value
                  || '');
              var content = options.template ? options.template(item) : item;
              return '<li role="listitem" aria-label="' + item
                  + '"><a href="#" data-value="' + value + '">' + icon(
                      options.checkClassName) + ' ' + content + '</a></li>';
            }).join('') : options.items;
        $node.html(markup).attr({'aria-label': options.title});
      });
  var palette = renderer.create('<div class="note-color-palette"/>',
      function ($node, options) {
        var contents = [];
        for (var row = 0, rowSize = options.colors.length; row < rowSize;
            row++) {
  ...
  var checkbox = renderer.create('<div class="checkbox"></div>',
      function ($node, options) {
        $node.html([
          '<label' + (options.id ? ' for="' + options.id + '"' : '') + '>',
          ' <input role="checkbox" type="checkbox"' + (options.id ? ' id="'
              + options.id + '"' : ''),
          ...
          function ($node, options) {
            if (options && options.tooltip) {
              $node.attr({
                ...
                container: (options.container !== undefined) ? options.container
                    : 'body',
                trigger: 'hover',
                placement: 'bottom'
              }).on('click', function (e) {
  ...
  function isFontInstalled(fontName) {
    var testFontName = fontName === 'Comic Sans MS' ? 'Courier New'
        : 'Comic Sans MS';
  ...
  var inputEventName = (isMSIE || isEdge)
      ? 'DOMCharacterDataModified DOMSubtreeModified DOMNodeInserted' : 'input';
  /**
   * @class core.env
   *
  ...
  function isVisiblePoint(point) {
    if (isText(point.node) || !hasChildren(point.node) || isEmpty$1(
        point.node)) {
      return true;
    }
  ...
  function isCustomStyleTag(node) {
    return node && !isText(node) && lists.contains(node.classList,
        'note-styletag');
  }
  ...
  var Context = /** @class */ (function () {
    ...
    Context.prototype._initialize = function () {
      ...
      var modules = $$1.extend({}, this.options.modules,
    ...
    Context.prototype.code = function (html) {
      ...
      if (html === undefined) {
        ...
        return isActivated ? this.layoutInfo.codable.val()
    ...
    Context.prototype.triggerEvent = function () {
      ...
      var callback = this.options.callbacks[func.namespaceToCamel(namespace,
          'on')];
      if (callback) {
     ...
     */
    Context.prototype.createInvokeHandlerAndUpdateState = function (namespace,
        value) {
      var _this = this;
    ...
    Context.prototype.createInvokeHandler = function (namespace, value) {
      ...
      return function (event) {
        ...
        _this.invoke(namespace,
            value || $target.closest('[data-value]').data('value'), $target);
      };
    };
  ...
  $$1.fn.extend({
    ...
    summernote: function () {
      ...
      var options = $$1.extend({}, $$1.summernote.options,
          hasInitOptions ? lists.head(arguments) : {});
      // Update options
      options.langInfo = $$1.extend(true, {}, $$1.summernote.lang['en-US'],
          $$1.summernote.lang[options.lang]);
      options.icons = $$1.extend(true, {}, $$1.summernote.options.icons,
          options.icons);
  ...
  function textRangeToPoint(textRange, isStart) {
    ...
    if (offset !== 0 && dom.isText(childNodes[offset - 1])) {
      ...
      curTextNode = prevContainer ? prevContainer.nextSibling
          : container.firstChild;
      var pointTester = textRange.duplicate();
      pointTester.setEndPoint('StartToStart', textRangeStart);
      var textCount = pointTester.text.replace(/[\r\n]/g, '').length;
      while (textCount > curTextNode.nodeValue.length
      ...
      if (isStart && curTextNode.nextSibling && dom.isText(
          curTextNode.nextSibling) &&
          textCount === curTextNode.nodeValue.length) {
        textCount -= curTextNode.nodeValue.length;
        curTextNode = curTextNode.nextSibling;
  ...
  var WrappedRange = /** @class */ (function () {
      ...
      var getVisiblePoint = function (point, isLeftToRight) {
        ...
        if (dom.isVisiblePoint(point)) {
          if (!dom.isEdgePoint(point) ||
              ...
              (dom.isRightEdgePoint(point) && isLeftToRight && dom.isVoid(
                  point.node.nextSibling)) ||
              (dom.isLeftEdgePoint(point) && !isLeftToRight && dom.isVoid(
        ...
        if (((dom.isLeftEdgePointOf(point, block) || dom.isVoid(
            dom.prevPoint(point).node)) && !isLeftToRight) ||
            ((dom.isRightEdgePointOf(point, block) || dom.isVoid(
                dom.nextPoint(point).node)) && isLeftToRight)) {
        ...
        var nextPoint = isLeftToRight ? dom.nextPointUntil(dom.nextPoint(point),
            dom.isVisiblePoint)
            : dom.prevPointUntil(dom.prevPoint(point), dom.isVisiblePoint);
        return nextPoint || point;
      };
      var endPoint = getVisiblePoint(this.getEndPoint(), false);
      var startPoint = this.isCollapsed() ? endPoint : getVisiblePoint(
    ...
    WrappedRange.prototype.expand = function (pred) {
      ...
      return new WrappedRange(boundaryPoints.sc, boundaryPoints.so,
          boundaryPoints.ec, boundaryPoints.eo);
    };
    /**
    ...
    WrappedRange.prototype.deleteContents = function () {
      ...
      return new WrappedRange(point.node, point.offset, point.node,
    ...
    WrappedRange.prototype.wrapBodyInlineWithPara = function () {
      ...
      var inlineSiblings = dom.listPrev(topAncestor,
  ...
  var Style = /** @class */ (function () {
    ...
    Style.prototype.fromNode = function ($node) {
      var properties = ['font-family', 'font-size', 'text-align',
    ...
    Style.prototype.current = function (rng) {
      ...
      try {
        styleInfo = $$1.extend(styleInfo, {
          ...
          'font-italic': document.queryCommandState('italic') ? 'italic'
              : 'normal',
      ...
      if (!rng.isOnList()) {
        ...
        var isUnordered = orderedTypes.indexOf(styleInfo['list-style-type'])
            > -1;
      ...
      if (para && para.style['line-height']) {
        ...
        var lineHeight = parseInt(styleInfo['line-height'], 10) / parseInt(
            styleInfo['font-size'], 10);
    ...
    Bullet.prototype.releaseList = function (clustereds, isEscapseToBody) {
      ...
      $$1.each(clustereds, function (idx, paras) {
        ...
        if (headList.parentNode.nodeName === 'LI') {
          ...
          var lastList = headList.childNodes.length > 1 ? dom.splitTree(
              headList, {
      ...
      if (splitRoot) {
        ...
        if (dom.isEmpty(splitRoot) && dom.isLi(splitRoot)) {
          ...
          if (blockquote) {
            ...
            if (dom.isRightEdgePoint(rng.getStartPoint()) && dom.isBR(
                rng.sc.nextSibling)) {
            ...
            if ((dom.isHeading(nextPara) || dom.isPre(nextPara)
                || dom.isCustomStyleTag(nextPara)) && dom.isEmpty(nextPara)) {
  ...
  var TableResultAction = function (startPoint, where, action, domTable) {
    ...
    function setStartPoint() {
      if (!startPoint || !startPoint.tagName
     ...
     */
    function setVirtualTablePosition(rowIndex, cellIndex, baseRow, baseCell,
        isRowSpan, isColSpan, isVirtualCell) {
      var objPosition = {
        'baseRow': baseRow,
        'baseCell': baseCell,
        'isRowSpan': isRowSpan,
    ...
    function getActionCell(virtualTableCellObj, resultAction,
        virtualRowPosition, virtualColPosition) {
      return {
    ...
    function addCellInfoToVirtual(row, cell) {
      ...
      setVirtualTablePosition(row.rowIndex, cellIndex, row, cell,
          cellHasRowspan, cellHasColspan, false);
      // Add span rows to virtual Table.
      var rowspanNumber = cell.attributes.rowSpan ? parseInt(
          cell.attributes.rowSpan.value, 10) : 0;
      if (rowspanNumber > 1) {
        for (var rp = 1; rp < rowspanNumber; rp++) {
          var rowspanIndex = row.rowIndex + rp;
          adjustStartPoint(rowspanIndex, cellIndex, cell, isThisSelectedCell);
          setVirtualTablePosition(rowspanIndex, cellIndex, row, cell, true,
              cellHasColspan, true);
        }
      }
      // Add span cols to virtual table.
      var colspanNumber = cell.attributes.colSpan ? parseInt(
          cell.attributes.colSpan.value, 10) : 0;
      if (colspanNumber > 1) {
        for (var cp = 1; cp < colspanNumber; cp++) {
          var cellspanIndex = recoverCellIndex(row.rowIndex, (cellIndex + cp));
          adjustStartPoint(row.rowIndex, cellspanIndex, cell,
              isThisSelectedCell);
          setVirtualTablePosition(row.rowIndex, cellspanIndex, row, cell,
              cellHasRowspan, true, true);
        }
      }
    }
    ...
    this.getActionList = function () {
      var fixedRow = (where === TableResultAction.where.Row)
          ? _startPoint.rowPos : -1;
      var fixedCol = (where === TableResultAction.where.Column)
          ? _startPoint.colPos : -1;
  ...
  var Table = /** @class */ (function () {
        ...
        switch (currentCell.action) {
          ...
          case TableResultAction.resultAction.SumSpanCount:
            if (position === 'top') {
              ...
              var isTopFromRowSpan = (!baseCellTr ? 0
                  : currentCell.baseCell.closest('tr').rowIndex)
                  <= currentTr[0].rowIndex;
              if (isTopFromRowSpan) {
                var newTd = $$1('<div></div>').append(
                    $$1('<td' + tdAttributes + '>' + dom.blank
    ...
    Table.prototype.addCol = function (rng, position) {
      ...
      var vTable = new TableResultAction(cell, TableResultAction.where.Column,
          TableResultAction.requestAction.Add, $$1(row).closest('table')[0]);
      var actions = vTable.getActionList();
    ...
    Table.prototype.deleteRow = function (rng) {
      ...
      var vTable = new TableResultAction(cell, TableResultAction.where.Row,
          TableResultAction.requestAction.Delete, $$1(row).closest('table')[0]);
      var actions = vTable.getActionList();
      for (var actionIndex = 0; actionIndex < actions.length; actionIndex++) {
      ...
      for (var actionIndex = 0; actionIndex < actions.length; actionIndex++) {
        ...
        switch (actions[actionIndex].action) {
          ...
          case TableResultAction.resultAction.SubtractSpanCount:
            ...
            if (hasColspan) {
              var colspanNumber = (baseCell.colSpan) ? parseInt(
                  baseCell.colSpan, 10) : 0;
              if (colspanNumber > 2) {
                colspanNumber--;
  ...
  var Editor = /** @class */ (function () {
    function Editor(context) {
      ...
      for (var idx = 0, len = commands.length; idx < len; idx++) {
        ...
        this.context.memo('help.' + commands[idx],
      ...
      for (var idx = 1; idx <= 6; idx++) {
        ...
        this.context.memo('help.formatH' + idx,
      ...
      this.formatBlock = this.wrapCommand(function (tagName, $target) {
        ...
        if (onApplyCustomStyle) {
          onApplyCustomStyle.call(_this, $target, _this.context,
              _this.onFormatBlock);
        } else {
      ...
      this.createLink = this.wrapCommand(function (linkInfo) {
        ...
        range.create(startPoint.node, startPoint.offset, endPoint.node,
            endPoint.offset).select();
        _this.setLastRange();
      });
      /**
       * setting color
    ...
    Editor.prototype.isLimited = function (pad, event) {
      ...
      if (this.options.maxTextLength > 0) {
        if ((this.$editable.text().length + pad)
    ...
    Editor.prototype.currentStyle = function () {
      ...
      return rng ? this.style.current(rng) : this.style.fromNode(
          this.$editable);
    };
    /**
     * style from node
    ...
    Editor.prototype.insertImagesAsDataURL = function (files) {
      ...
      $$1.each(files, function (idx, file) {
        ...
        if (_this.options.maximumImageFileSize
  ...
  var Clipboard = /** @class */ (function () {
    ...
    Clipboard.prototype.pasteByEvent = function (event) {
      ...
      if (clipboardData && clipboardData.items && clipboardData.items.length) {
        // paste img file
        var item = clipboardData.items.length > 1 ? clipboardData.items[1]
  ...
  var Dropzone = /** @class */ (function () {
    ...
    Dropzone.prototype.attachDragAndDropEvent = function () {
      ...
      this.documentEventHandlers.onDragenter = function (e) {
        ...
        var hasEditorSize = _this.$editor.width() > 0 && _this.$editor.height()
            > 0;
    ...
    Dropzone.prototype.destroy = function () {
      ...
      Object.keys(this.documentEventHandlers).forEach(function (key) {
        _this.$eventListener.off(key.substr(2).toLowerCase(),
  ...
  var CodeView = /** @class */ (function () {
        ...
        if (this.options.codeviewIframeFilter) {
          ...
          value = value.replace(/(<iframe.*?>.*?(?:<\/iframe>)?)/gi,
              function (tag) {
                ...
                if (/<.+src(?==?('|"|\s)?)[\s\S]+src(?=('|"|\s)?)[^>]*?>/i.test(
                    tag)) {
                  return '';
                }
                for (var _i = 0, whitelist_2 = whitelist_1;
                    _i < whitelist_2.length; _i++) {
                  var src = whitelist_2[_i];
                  // pass if src is trusted
                  if ((new RegExp('src="(https?:)?\/\/' + src.replace(
                      /[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + '\/(.+)"')).test(
                      tag)) {
                    return tag;
                  }
    ...
    CodeView.prototype.activate = function () {
      ...
      if (env.hasCodeMirror) {
        var cmEditor_1 = CodeMirror.fromTextArea(this.$codable[0],
        ...
        cmEditor_1.on('blur', function (event) {
          _this.context.triggerEvent('blur.codeview', cmEditor_1.getValue(),
              event);
        });
        cmEditor_1.on('change', function (event) {
          _this.context.triggerEvent('change.codeview', cmEditor_1.getValue(),
              cmEditor_1);
        });
        ...
        this.$codable.on('blur', function (event) {
          _this.context.triggerEvent('blur.codeview', _this.$codable.val(),
              event);
        });
        this.$codable.on('input', function (event) {
          _this.context.triggerEvent('change.codeview', _this.$codable.val(),
              _this.$codable);
        });
      }
    };
  ...
  var Statusbar = /** @class */ (function () {
    ...
    Statusbar.prototype.initialize = function () {
      ...
      this.$statusbar.on('mousedown', function (event) {
        ...
        var editableTop = _this.$editable.offset().top
            - _this.$document.scrollTop();
        var onMouseMove = function (event) {
          var height = event.clientY - (editableTop + EDITABLE_PADDING);
          height = (_this.options.minheight > 0) ? Math.max(height,
              _this.options.minheight) : height;
          height = (_this.options.maxHeight > 0) ? Math.min(height,
              _this.options.maxHeight) : height;
          _this.$editable.height(height);
        };
        _this.$document.on('mousemove', onMouseMove).one('mouseup',
            function () {
  ...
  var Handle = /** @class */ (function () {
    ...
    Handle.prototype.initialize = function () {
      ...
      this.$handle = $$1([
        ...
        '<div class="',
        (this.options.disableResizeImage ? 'note-control-holder'
            : 'note-control-sizing'),
        ' note-control-se"></div>',
      ...
      this.$handle.on('mousedown', function (event) {
        if (dom.isControlSizing(event.target)) {
          event.preventDefault();
          event.stopPropagation();
          var $target_1 = _this.$handle.find('.note-control-selection').data(
              'target');
    ...
    Handle.prototype.update = function (target, event) {
      ...
      if (isImage) {
        ...
        var sizingText = imageSize.w + 'x' + imageSize.h + ' ('
            + this.lang.image.original + ': ' + origImageObj.width + 'x'
            + origImageObj.height + ')';
  ...
  var AutoLink = /** @class */ (function () {
    ...
    AutoLink.prototype.handleKeydown = function (e) {
      if (lists.contains([key.code.ENTER, key.code.SPACE], e.keyCode)) {
        var wordRange = this.context.invoke(
            'editor.createRange').getWordRange();
  ...
  var AutoReplace = /** @class */ (function () {
    function AutoReplace(context) {
      ...
      this.keys = [key.code.ENTER, key.code.SPACE, key.code.PERIOD,
        key.code.COMMA, key.code.SEMICOLON, key.code.SLASH];
    ...
    AutoReplace.prototype.handleKeydown = function (e) {
      ...
      if (this.previousKeydownCode && lists.contains(this.keys,
  ...
  var Buttons = /** @class */ (function () {
    ...
    Buttons.prototype.isFontDeservedToAdd = function (name) {
      var genericFamilies = ['sans-serif', 'serif', 'monospace', 'cursive',
        'fantasy'];
      name = name.toLowerCase();
      return (name !== '' && this.isFontInstalled(name)
          && genericFamilies.indexOf(name) === -1);
    };
    Buttons.prototype.colorPalette = function (className, tooltip, backColor,
        foreColor) {
      var _this = this;
        ...
        children: [
          ...
          this.ui.dropdown({
            items: (backColor ? [
                  ...
                  '  <div class="note-palette-title">' + this.lang.color.background
                  + '</div>',
                  '  <div>',
                ...
                (foreColor ? [
                  ...
                  + this.lang.color.foreground + '</div>',
                  '  <div>',
            ...
            callback: function ($dropdown) {
              ...
              $dropdown.find('input[type=color]').each(function (idx, item) {
                $$1(item).change(function () {
                  var $chip = $dropdown.find(
                      '#' + $$1(this).data('event')).find(
                      '.note-color-btn').first();
            ...
            click: function (event) {
              ...
              if (eventName === 'openPalette') {
                ...
                var $palette = $$1(
                    $parent.find('#' + $picker.data('event')).find(
                        '.note-color-row')[0]);
                // Shift palette chips
                ...
                $picker.click();
              } else if (lists.contains(['backColor', 'foreColor'],
                  eventName)) {
                var key = eventName === 'backColor' ? 'background-color'
                    : 'color';
                var $color = $button.closest('.note-color').find(
                    '.note-recent-color');
                var $currentButton = $button.closest('.note-color').find(
    ...
    Buttons.prototype.addToolbarButtons = function () {
        ...
        return _this.ui.buttonGroup([
          _this.button({
            ...
            contents: _this.ui.dropdownButtonContents(
                _this.ui.icon(_this.options.icons.magic), _this.options),
      ...
      var this_1 = this;
      for (var styleIdx = 0, styleLen = this.options.styleTags.length;
          styleIdx < styleLen; styleIdx++) {
      ...
      this.context.memo('button.underline', function () {
        return _this.button({
          ...
          tooltip: _this.lang.font.underline + _this.representShortcut(
              'underline'),
          click: _this.context.createInvokeHandlerAndUpdateState(
              'editor.underline')
        }).render();
      ...
      this.context.memo('button.clear', function () {
        return _this.button({
          ...
          tooltip: _this.lang.font.clear + _this.representShortcut(
              'removeFormat'),
      ...
      this.context.memo('button.strikethrough', function () {
        return _this.button({
          ...
          tooltip: _this.lang.font.strikethrough + _this.representShortcut(
              'strikethrough'),
          click: _this.context.createInvokeHandlerAndUpdateState(
      ...
      this.context.memo('button.fontname', function () {
        ...
        return _this.ui.buttonGroup([
          _this.button({
            className: 'dropdown-toggle',
            contents: _this.ui.dropdownButtonContents(
                '<span class="note-current-fontname"/>', _this.options),
          ...
          _this.ui.dropdownCheck({
            ...
            template: function (item) {
              return '<span style="font-family: \'' + item + '\'">' + item
                  + '</span>';
            },
            click: _this.context.createInvokeHandlerAndUpdateState(
                'editor.fontName')
      ...
      this.context.memo('button.fontsize', function () {
        return _this.ui.buttonGroup([
          _this.button({
            className: 'dropdown-toggle',
            contents: _this.ui.dropdownButtonContents(
                '<span class="note-current-fontsize"/>', _this.options),
      ...
      this.context.memo('button.color', function () {
        return _this.colorPalette('note-color-all', _this.lang.color.recent,
            true, true);
      });
      this.context.memo('button.forecolor', function () {
        return _this.colorPalette('note-color-fore',
            _this.lang.color.foreground, false, true);
      });
      this.context.memo('button.backcolor', function () {
        return _this.colorPalette('note-color-back',
            _this.lang.color.background, true, false);
      });
      this.context.memo('button.ul', function () {
        return _this.button({
          contents: _this.ui.icon(_this.options.icons.unorderedlist),
          tooltip: _this.lang.lists.unordered + _this.representShortcut(
              'insertUnorderedList'),
      ...
      this.context.memo('button.ol', function () {
        return _this.button({
          ...
          tooltip: _this.lang.lists.ordered + _this.representShortcut(
              'insertOrderedList'),
      ...
      var justifyLeft = this.button({
        ...
        tooltip: this.lang.paragraph.left + this.representShortcut(
            'justifyLeft'),
        click: this.context.createInvokeHandler('editor.justifyLeft')
      });
      var justifyCenter = this.button({
        contents: this.ui.icon(this.options.icons.alignCenter),
        tooltip: this.lang.paragraph.center + this.representShortcut(
            'justifyCenter'),
        click: this.context.createInvokeHandler('editor.justifyCenter')
      });
      var justifyRight = this.button({
        contents: this.ui.icon(this.options.icons.alignRight),
        tooltip: this.lang.paragraph.right + this.representShortcut(
            'justifyRight'),
        click: this.context.createInvokeHandler('editor.justifyRight')
      });
      var justifyFull = this.button({
        contents: this.ui.icon(this.options.icons.alignJustify),
        tooltip: this.lang.paragraph.justify + this.representShortcut(
            'justifyFull'),
        click: this.context.createInvokeHandler('editor.justifyFull')
      });
      var outdent = this.button({
        contents: this.ui.icon(this.options.icons.outdent),
        tooltip: this.lang.paragraph.outdent + this.representShortcut(
            'outdent'),
      ...
      this.context.memo('button.paragraph', function () {
        return _this.ui.buttonGroup([
          _this.button({
            className: 'dropdown-toggle',
            contents: _this.ui.dropdownButtonContents(
                _this.ui.icon(_this.options.icons.alignLeft), _this.options),
      ...
      this.context.memo('button.height', function () {
        return _this.ui.buttonGroup([
          _this.button({
            className: 'dropdown-toggle',
            contents: _this.ui.dropdownButtonContents(
                _this.ui.icon(_this.options.icons.textHeight), _this.options),
      ...
      this.context.memo('button.table', function () {
        return _this.ui.buttonGroup([
          _this.button({
            className: 'dropdown-toggle',
            contents: _this.ui.dropdownButtonContents(
                _this.ui.icon(_this.options.icons.table), _this.options),
      ...
      this.context.memo('button.link', function () {
        return _this.button({
          ...
          tooltip: _this.lang.link.link + _this.representShortcut(
              'linkDialog.show'),
      ...
      this.context.memo('button.hr', function () {
        return _this.button({
          ...
          tooltip: _this.lang.hr.insert + _this.representShortcut(
    ...
    Buttons.prototype.build = function ($container, groups) {
      for (var groupIdx = 0, groupLen = groups.length; groupIdx < groupLen;
          groupIdx++) {
        var group = groups[groupIdx];
        var groupName = Array.isArray(group) ? group[0] : group;
        var buttons = Array.isArray(group) ? ((group.length === 1) ? [group[0]]
            : group[1]) : [group];
    ...
    Buttons.prototype.updateCurrentStyle = function ($container) {
      ...
      if (styleInfo['font-family']) {
        var fontNames = styleInfo['font-family'].split(',').map(
            function (name) {
        ...
        $cont.find('.note-current-fontname').text(fontName_1).css('font-family',
            fontName_1);
      }
  ...
  var Toolbar = /** @class */ (function () {
    ...
    Toolbar.prototype.initialize = function () {
      ...
      if (!this.options.toolbar.length) {
        ...
        this.context.invoke('buttons.build', this.$toolbar,
    ...
    Toolbar.prototype.followScroll = function () {
      ...
      var deactivateOffsetBottom = editorOffsetBottom - otherBarHeight
          - toolbarHeight - statusbarHeight;
      if (!this.isFollowing &&
          (currentOffset > activateOffset) && (currentOffset
    ...
    Toolbar.prototype.updateFullscreen = function (isFullscreen) {
      this.ui.toggleBtnActive(this.$toolbar.find('.btn-fullscreen'),
          isFullscreen);
  ...
  var LinkDialog = /** @class */ (function () {
    ...
    LinkDialog.prototype.initialize = function () {
      ...
      var body = [
        ...
        "<label class=\"note-form-label\">" + this.lang.link.textToDisplay
        + "</label>",
      ...
      var footer = "<input type=\"button\" href=\"#\" class=\"" + buttonClass
          + "\" value=\"" + this.lang.link.insert + "\" disabled>";
     ...
     */
    LinkDialog.prototype.toggleLinkBtn = function ($linkBtn, $linkText,
        $linkUrl) {
  ...
  var ImageDialog = /** @class */ (function () {
    ...
    ImageDialog.prototype.initialize = function () {
      ...
      if (this.options.maximumImageFileSize) {
        ...
        var readableSize = (this.options.maximumImageFileSize / Math.pow(1024,
            unit)).toFixed(2) * 1 +
            ' ' + ' KMGTP'[unit] + 'B';
      ...
      var body = [
        ...
        '<label class="note-form-label">' + this.lang.image.selectFromFiles
        + '</label>',
      ...
      var footer = "<input type=\"button\" href=\"#\" class=\"" + buttonClass
          + "\" value=\"" + this.lang.image.insert + "\" disabled>";
      this.$dialog = this.ui.dialog({
      ...
      return $$1.Deferred(function (deferred) {
        ...
        _this.ui.onDialogShown(_this.$dialog, function () {
          ...
          $imageInput.replaceWith(
              $imageInput.clone().on('change', function (event) {
  ...
  var ImagePopover = /** @class */ (function () {
    ...
    ImagePopover.prototype.initialize = function () {
      ...
      var $content = this.$popover.find(
          '.popover-content,.note-popover-content');
      this.context.invoke('buttons.build', $content,
    ...
    ImagePopover.prototype.update = function (target, event) {
      if (dom.isImg(target)) {
        ...
        this.$popover.css({
          ...
          top: this.options.popatmouse ? event.pageY : Math.min(pos.top,
              posEditor.top)
        });
      } else {
        this.hide();
  ...
  var VideoDialog = /** @class */ (function () {
    ...
    VideoDialog.prototype.initialize = function () {
      ...
      var body = [
        ...
        "<label class=\"note-form-label\">" + this.lang.video.url
        + " <small class=\"text-muted\">" + this.lang.video.providers
        + "</small></label>",
    ...
    VideoDialog.prototype.createVideoNode = function (url) {
      ...
      if (ytMatch && ytMatch[1].length === 11) {
        ...
        .attr('frameborder', 0)
        .attr('src',
            '//www.youtube.com/embed/' + youtubeId + (start > 0 ? '?start='
            + start : ''))
        ...
        $video = $$1('<iframe>')
        .attr('frameborder', 0)
        .attr('src', 'https://www.facebook.com/plugins/video.php?href='
            + encodeURIComponent(fbMatch[0]) + '&show_text=0&width=560')
    ...
    function AirPopover(context) {
      ...
      this.events = {
        ...
        'summernote.focusout': function (we, e) {
          ...
          if (!e.relatedTarget || !dom.ancestor(e.relatedTarget,
  ...
  var HintPopover = /** @class */ (function () {
    ...
    HintPopover.prototype.handleKeyup = function (e) {
      var _this = this;
      if (!lists.contains([key.code.ENTER, key.code.UP, key.code.DOWN],
          e.keyCode)) {
        var wordRange = this.context.invoke(
            'editor.getLastRange').getWordRange();
  ...
  $$1.summernote = $$1.extend($$1.summernote, {
    ...
    options: {
      ...
      colorsName: [
        ['Black', 'Tundora', 'Dove Gray', 'Star Dust', 'Pale Slate', 'Gallery',
          'Alabaster', 'White'],
        ['Red', 'Orange Peel', 'Yellow', 'Green', 'Cyan', 'Blue',
          'Electric Violet', 'Magenta'],
        ['Azalea', 'Karry', 'Egg White', 'Zanah', 'Botticelli', 'Tropical Blue',
          'Mischka', 'Twilight'],
        ...
        ['Sangria', 'Mai Tai', 'Buddha Gold', 'Forest Green', 'Eden',
          'Venice Blue', 'Meteorite', 'Claret'],
        ['Rosewood', 'Cinnamon', 'Olive', 'Parsley', 'Tiber', 'Midnight Blue',
          'Valentino', 'Loulou'],
      ],
      colorButton: {
        foreColor: '#000000',
