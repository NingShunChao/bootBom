...
(function ($) {
  $.fn.tableExport = function (options) {
    ...
    if (defaults.type == 'csv' || defaults.type == 'tsv' || defaults.type
        == 'txt') {
      ...
      function csvString(cell, rowIndex, colIndex) {
        ...
        if (cell !== null) {
          ...
          var csvValue = (dataString === null || dataString === '') ? ''
              : dataString.toString();
          ...
          if (defaults.type == 'tsv') {
            ...
            if (dataString instanceof Date) {
              result = defaults.csvEnclosure + dataString.toLocaleString()
                  + defaults.csvEnclosure;
            } else {
              result = replaceAll(csvValue, defaults.csvEnclosure,
        ...
        $rows.each(function () {
          ...
          ForEachVisibleCell(this, rowselector, rowIndex, length + $rows.length,
              function (cell, row, col) {
                trData += csvString(cell, row, col) + (defaults.type == 'tsv'
      ...
      rowlength += CollectCsvData(
          $(el).find('thead').first().find(defaults.theadSelector), 'th,td',
          rowlength);
      findTablePart($(el), 'tbody').each(function () {
        rowlength += CollectCsvData(findRows($(this), defaults.tbodySelector),
            'td,th', rowlength);
      });
      if (defaults.tfootSelector.length) {
        CollectCsvData($(el).find('tfoot').first().find(defaults.tfootSelector),
            'td,th', rowlength);
      }
      ...
      if (defaults.outputMode === 'window') {
        downloadFile(false,
            'data:text/' + (defaults.type == 'csv' ? 'csv' : 'plain')
            + ';charset=utf-8,', csvData);
        return;
      }

      try {
        blob = new Blob([csvData], {
          type: "text/" + (defaults.type == 'csv' ? 'csv' : 'plain')
              + ";charset=utf-8"
        });
        ...
        downloadFile(defaults.fileName + '.' + defaults.type,
            'data:text/' + (defaults.type == 'csv' ? 'csv' : 'plain')
            + ';charset=utf-8,' + ((defaults.type == 'csv'
            && defaults.csvUseBOM) ? '\ufeff' : ''),
            csvData);
      }
      ...
      $($rows).each(function () {
        trData = "";
        ForEachVisibleCell(this, 'td,th', rowIndex,
        ...
        ForEachVisibleCell(this, 'td,th', rowIndex,
            ...
            function (cell, row, col) {
              if (jsonHeaderArray.length) {
                jsonObjectTd[jsonHeaderArray[jsonHeaderArray.length
                - 1][colIndex]] = parseString(cell, row, col);
              } else {
      ...
      $($rows).each(function () {
        ...
        ForEachVisibleCell(this, 'td,th', rowIndex,
            $hrows.length + $rows.length,
            function (cell, row, col) {
              trData += "<column-" + colCount + ">" + parseString(cell, row,
                  col) + "</column-" + colCount + ">";
              colCount++;
            });
    ...
    } else if (defaults.type === 'excel' && defaults.excelFileFormat
        === 'xmlss') {
      var docDatas = [];
      var docNames = [];
      ...
      $(el).filter(function () {
        ...
        var ssName = '';
        if (typeof defaults.worksheetName === 'string'
            && defaults.worksheetName.length) {
          ssName = defaults.worksheetName + ' ' + (docNames.length + 1);
        } else if (typeof defaults.worksheetName[docNames.length]
            !== 'undefined') {
                  ...
                  if (cell !== null) {
                    ...
                    $.each(spans, function () {
                      ...
                      if (rowIndex >= range.s.r && rowIndex <= range.e.r
                          && nCols >= range.s.c && nCols <= range.e.c) {
        ...
        rowLength += CollectXmlssData(
            $table.find('thead').first().find(defaults.theadSelector), 'th,td',
            rowLength);
      ...
      for (var n = 0, c = docNames.length; n < c; n++) {
        ...
        if (itemCount == 2) {
          docNames[firstOccurences[item]] = docNames[firstOccurences[item]].substring(
              0, 29) + "-1";
        }
        if (count[item] > 1) {
      ...
      for (var j = 0; j < docDatas.length; j++) {
        xmlssDocFile += '<Worksheet ss:Name="' + docNames[j]
            + '" ss:RightToLeft="' + (defaults.excelRTL ? '1' : '0') + '">\r' +
            docDatas[j];
      ...
      var MSDocType = (defaults.type == 'excel' || defaults.type == 'xls')
          ? 'excel' : 'word';
      ...
      $(el).filter(function () {
        ...
        if (docName === '') {
          docName = defaults.worksheetName || $table.find('caption').text()
              || 'Table';
          docName = $.trim(
          ...
          ForEachVisibleCell(this, 'th,td', rowIndex, $hrows.length,
              function (cell, row, col) {
                if (cell !== null) {
                  ...
                  if (typeof tdcolspan == 'undefined' && $(cell).is(
                      "[colspan]")) {
                  ...
                  if (typeof tdrowspan == 'undefined' && $(cell).is(
                      "[rowspan]")) {
        ...
        if (defaults.displayTableName) {
          docData += '<tr><td></td></tr><tr><td></td></tr><tr><td>'
      ...
      var docFile = '<html xmlns:o="urn:schemas-microsoft-com:office:office" '
          + MSDocSchema + ' xmlns="http://www.w3.org/TR/REC-html40">';
      docFile += '<meta http-equiv="content-type" content="application/vnd.ms-'
          + MSDocType + '; charset=UTF-8">';
      docFile += "<head>";
      if (MSDocType === 'excel') {
      ...
      $rows = $(el).find('thead').first().find(
          defaults.theadSelector).toArray();
            ...
            function (cell, row, col) {
              if (typeof cell !== 'undefined' && cell !== null) {
                ...
                $.each(spans, function () {
                  var range = this;
                  if (rowIndex >= range.s.r && rowIndex <= range.e.r
                      && cols.length >= range.s.c && cols.length <= range.e.c) {
                    for (var i = 0; i <= range.e.c - range.s.c; ++i) {
                      cols.push(null);
                    }
                  }
      ...
      var wbout = XLSX.write(wb,
          {bookType: defaults.type, bookSST: false, type: 'binary'});
      ...
      if (defaults.pdfmake.enabled === true) {
        ...
        var doc = new jsPDF(defaults.jspdf.orientation, defaults.jspdf.unit,
        ...
        if (typeof defaults.jspdf.format === 'string'
        ...
        if (typeof teOptions.images != 'undefined') {
          $(el).filter(function () {
            ...
            if (defaults.exportHiddenCells === false) {
              $hiddenTableElements = $(this).find("tr, th, td").filter(
                  ":hidden");
              checkCellVisibilty = $hiddenTableElements.length > 0;
            }
            ...
            $($rows).each(function () {
              ForEachVisibleCell(this, 'td,th', $hrows.length + rowCount,
                  $hrows.length + $rows.length,
                  function (cell) {
                    if (typeof cell !== 'undefined' && cell !== null) {
          ...
          $(el).filter(function () {
              ...
              atOptions.beforePageContent = function (data) {
                if (data.pageCount == 1) {
                  ...
                  $.each(all, function () {
                    var row = this;
                    if (row.height > 0) {
                      row.height += (2 - FONT_ROW_RATIO) / 2
                          * row.styles.fontSize;
                      data.table.height += (2 - FONT_ROW_RATIO) / 2
                          * row.styles.fontSize;
                    }
                  });
                }
            ...
            if (typeof atOptions.createdHeaderCell !== 'function') {
              ...
              atOptions.createdHeaderCell = function (cell, data) {
                ...
                if (typeof teOptions.columns [data.column.dataKey]
                    != 'undefined') {
                  ...
                  if (typeof col.rect != 'undefined') {
                    ...
                    if (typeof teOptions.heightRatio == 'undefined'
                  ...
                  if (typeof col.style != 'undefined' && col.style.hidden
                      !== true) {
            ...
            if (typeof atOptions.createdCell !== 'function') {
              ...
              atOptions.createdCell = function (cell, data) {
                var rowopt = teOptions.rowoptions [data.row.index + ":"
                + data.column.dataKey];
            ...
            if (typeof atOptions.drawHeaderCell !== 'function') {
              atOptions.drawHeaderCell = function (cell, data) {
                ...
                if ((colopt.style.hasOwnProperty("hidden") !== true
            ...
            if (typeof atOptions.drawCell !== 'function') {
              atOptions.drawCell = function (cell, data) {
                ...
                if (prepareAutoTableText(cell, data, rowopt)) {
                  ...
                  teOptions.doc.rect(cell.x, cell.y, cell.width, cell.height,
                      cell.styles.fillStyle);

                  if (typeof rowopt != 'undefined' && typeof rowopt.kids
                      != 'undefined' && rowopt.kids.length > 0) {

                    var dh = cell.height / rowopt.rect.height;
                    if (dh > teOptions.dh || typeof teOptions.dh
                        == 'undefined') {
                      teOptions.dh = dh;
                    }
            ...
            if (typeof teOptions.onBeforeAutotable === 'function') {
              teOptions.onBeforeAutotable($(this), teOptions.columns,
                  teOptions.rows, atOptions);
            }

            teOptions.doc.autoTable(teOptions.columns, teOptions.rows,
                atOptions);
            ...
            defaults.jspdf.autotable.startY = teOptions.doc.autoTableEndPosY()
                + atOptions.margin.top;

          });

          jsPdfOutput(teOptions.doc,
              (typeof teOptions.images != 'undefined' && jQuery.isEmptyObject(
                  teOptions.images) === false));
    ...
    */
    ...
    function findRows($tpart, rowSelector) {
      return $tpart.find(rowSelector).filter(function () {
        return $(this).find('table').length === 0 && $(this).parents(
            'table').length === 1;
      });
    }
    ...
    function isVisible($element) {
      ...
      var isElementVisible = (isCell || isRow) ? isTableElementVisible($element)
          : $element.is(':visible');
      var tableexportDisplay = $element.data("tableexport-display");

      if (isCell && tableexportDisplay != 'none' && tableexportDisplay
          != 'always') {
        $element = $($element[0].parentNode);
        isRow = typeof $element[0].rowIndex !== 'undefined';
        tableexportDisplay = $element.data("tableexport-display");
      }
      if (isRow && tableexportDisplay != 'none' && tableexportDisplay
          != 'always') {
        tableexportDisplay = $element.closest('table').data(
            "tableexport-display");
      }

      return tableexportDisplay !== 'none' && (isElementVisible == true
      ...
      if (checkCellVisibilty) {
        hiddenEls = $hiddenTableElements.filter(function () {
          ...
          if (this.nodeType == $element[0].nodeType) {
            if (typeof this.rowIndex !== 'undefined' && this.rowIndex
                == $element[0].rowIndex) {
              found = true;
            } else if (typeof this.cellIndex !== 'undefined' && this.cellIndex
                == $element[0].cellIndex &&
      ...
      if (isVisible($cell)) {
        if (defaults.ignoreColumn.length > 0) {
          if ($.inArray(colIndex, defaults.ignoreColumn) != -1 ||
              ...
              (colNames.length > colIndex && typeof colNames[colIndex]
                  != 'undefined' &&
    ...
    function ForEachVisibleCell(tableRow, selector, rowIndex, rowCount,
        cellcallback) {
        ...
        if (ignoreRow === false &&
          ...
          $cells.each(function (colIndex) {
            ...
            $.each(ranges, function () {
              var range = this;
              if (rowIndex >= range.s.r && rowIndex <= range.e.r && cellCount
                  >= range.s.c && cellCount <= range.e.c) {
    ...
    function prepareAutoTableText(cell, data, cellopt) {
      ...
      if (cs >= 0) {
        ...
        if (cell.styles.valign === 'middle' || cell.styles.valign
            === 'bottom') {
          var splittedText = typeof cell.text === 'string' ? cell.text.split(
              /\r\n|\r|\n/g) : cell.text;
          var lineCount = splittedText.length || 1;
          if (lineCount > 2) {
            cell.textPos.y -= ((2 - FONT_ROW_RATIO) / 2
                * data.row.styles.fontSize) * (lineCount - 2) / 3;
          }
        }
        return true;
      } else {
    ...
    function drawAutotableElements(cell, elements, teOptions) {
      elements.each(function () {
        ...
        if ($(this).is("div")) {
          ...
          var lwidth = getPropertyUnitValue(this, 'border-top-width',
              defaults.jspdf.unit);
          ...
          teOptions.doc.rect(cell.x + ux, cell.y + uy, uw, uh,
              lwidth ? "FD" : "F");
        } else if ($(this).is("img")) {
          if (typeof teOptions.images != 'undefined') {
            var hash = strHashCode(this.src);
            ...
            if (typeof image != 'undefined') {
              ...
              try {
                teOptions.doc.addImage(image.src, cell.textPos.x, cell.y + uy,
                    imgWidth, imgHeight);
              } catch (e) {
      ...
      if (typeof teOptions.onAutotableText === 'function') {
        ...
        if (texttags.length) {
          ...
          while (tag) {
            ...
            var w = teOptions.doc.getStringUnitWidth(txt)
                * teOptions.doc.internal.getFontSize();

            if (w) {
              if (cell.styles.overflow === 'linebreak' &&
                  x > cell.textPos.x && (x + w) > (cell.textPos.x
                      + cell.width)) {
                var chars = ".,!%*;:=-";
                if (chars.indexOf(txt.charAt(0)) >= 0) {
                  var s = txt.charAt(0);
                  w = teOptions.doc.getStringUnitWidth(s)
                      * teOptions.doc.internal.getFontSize();
                  if ((x + w) <= (cell.textPos.x + cell.width)) {
                    teOptions.doc.autoTableText(s, x, y, style);
                    txt = txt.substring(1, txt.length);
                  }
                  w = teOptions.doc.getStringUnitWidth(txt)
                      * teOptions.doc.internal.getFontSize();
                }
                x = cell.textPos.x;
                y += teOptions.doc.internal.getFontSize();
      ...
      if (cell !== null) {
        ...
        if ($cell[0].hasAttribute("data-tableexport-value")) {
          ...
          if (typeof defaults.onCellHtmlData === 'function') {
            htmlData = defaults.onCellHtmlData($cell, rowIndex, colIndex,
                htmlData);
          } else if (htmlData != '') {
            ...
            $.each(html, function () {
              if ($(this).is("input")) {
                ...
                htmlData += $cell.find('select option:selected').eq(
                    selectidx++).text();
              } else {
        ...
        if (defaults.htmlContent === true) {
          ...
          if (cellFormat != '') {
            ...
            if (defaults.type == 'json' ||
                (defaults.type === 'excel' && defaults.excelFileFormat
                    === 'xmlss') ||
            ...
            } else if (defaults.numbers.html.decimalMark
              ...
              if (number !== false) {
                ...
                result = (number < 0 ? "-" : "") +
                    (defaults.numbers.output.thousandsSeparator ? ((mod
                        ? frac[0].substr(0, mod)
                        + defaults.numbers.output.thousandsSeparator : "")
                        + frac[0].substr(mod).replace(/(\d{3})(?=\d)/g,
                            "$1" + defaults.numbers.output.thousandsSeparator))
                        : frac[0]) +
                    (frac[1].length ? defaults.numbers.output.decimalMark
                        + frac[1] : "");
              }
            }
          } else {
    ...
    function downloadFile(filename, header, data) {
      ...
      } else if (filename !== false && (ua.indexOf("MSIE ") > 0 || !!ua.match(
          /Trident.*rv\:11\./))) {
