    ...
    if (this.options.showExport) {
      ...
      if (!$export.length) {
        ...
        if (typeof this.options.exportTypes === 'string') {
          var types = this.options.exportTypes.slice(1, -1).replace(/ /g,
              '').split(',');
        ...
        $menu.find('li').click(function () {
          ...
          if (that.options.exportDataType === 'all'
              && that.options.pagination) {
            that.$el.one(
                that.options.sidePagination === 'server' ? 'post-body.bs.table'
            ...
            for (var i = 0; i < trs.length; i++) {
              var $this = $(trs[i]);
              if (!$this.find(
                  sprintf('[name="%s"]', that.options.selectItemName)).prop(
                  'checked')) {
                $this['hide']();
              }
            }
