...
(function ($) {
  ...
  BootstrapTable.prototype.initHeader = function () {
    ...
    if (this.options.rightFixedColumns) {
      for (var i = 0; i < this.options.rightFixedNumber; i++) {
        $rtr.append(
            $rths.eq($rths.length - this.options.rightFixedNumber + i).clone());
      }
    ...
    if (this.options.fixedColumns) {
      ...
      this.$selectAll.on('click', function () {
        ...
        $(".left-fixed-body-columns input[name=btSelectItem]").filter(
            ':enabled').prop('checked', checked);
        $('.fixed-table-body input[name=btSelectItem]').closest(
      ...
      this.$body.find('> tr[data-index]').each(function () {
        ...
        for (var i = 0; i < that.options.rightFixedNumber; i++) {
          ...
          buttons.each(function (key, item) {
            $(item).click(function () {
              that.$body.find(
                  "tr[data-index=" + $tr.attr('data-index') + "] td:eq("
                  + indexTd + ") button:eq(" + key + ")").click();
            });
          });
          $tr.append(fixTd);
        }
  ...
  BootstrapTable.prototype.resetView = function () {
    ...
    this.timeoutHeaderColumns_ = setTimeout(
        $.proxy(this.fitHeaderColumns, this), this.$el.is(':hidden') ? 100 : 0);

    clearTimeout(this.timeoutBodyColumns_);
    this.timeoutBodyColumns_ = setTimeout($.proxy(this.fitBodyColumns, this),
      ...
      this.$body.find('tr:first-child:not(.no-records-found) > *').each(
          function (i) {
            ...
            that.$fixedHeader.find(
                'thead th[data-field="' + visibleFields[index] + '"]')
  ...
  BootstrapTable.prototype.fitBodyColumns = function () {
    ...
    if (that.options.fixedColumns) {
      ...
      $("#" + table.options.id).on("check.bs.table uncheck.bs.table",
          function (e, rows, $element) {
            var index = $element.data('index');
            $(this).find('.bs-checkbox').find(
            ...
            $(".left-fixed-table-columns input[name=btSelectAll]").prop(
                'checked', checkAll);
            $('.fixed-table-body input[name=btSelectItem]').closest(
                'tr').removeClass('selected');
          });

      $("#" + table.options.id).off('click', '.fixed-table-body').on('click',
          '.th-inner', function (event) {
      ...
      // events
      this.$fixedHeader.off('click', '.th-inner').on('click', '.th-inner',
          function (event) {
            var target = $(this);
            var $this = event.type === "keypress" ? $(event.currentTarget) : $(
                event.currentTarget).parent(),
                $this_ = that.$header.find('th').eq($this.index());
            ...
            var sortName = $this.data('sortName') ? $this.data('sortName')
                : $this.data('field');
            if (target.parent().data().sortable) {
              $.each(that.$fixedHeader.find('th'), function (i, th) {
                $(th).find('.sortable').removeClass('desc asc').addClass(
                    ($(th).data('field') === sortName || $(th).data('sortName')
                        === sortName) ? sortOrder : 'both');
              });
            }
          });
      ...
      this.$body.find('> tr[data-index]').off('hover').hover(function () {
        ...
        that.$fixedBody.find('tr[data-index="' + index + '"]').addClass(
            'hover');
      }, function () {
        var index = $(this).data('index');
        that.$fixedBody.find('tr[data-index="' + index + '"]').removeClass(
            'hover');
      });
      this.$fixedBody.find('tr[data-index]').off('hover').hover(function () {
        ...
        var index = $(this).data('index');
        that.$body.find('> tr[data-index="' + index + '"]').removeClass(
            'hover');
      });
    }
    if (that.options.rightFixedColumns) {
      ...
      this.$body.find('> tr').each(function (i) {
        that.$rightfixedBody.find('tbody tr:eq(' + i + ')').height(
            $(this).height());
      });
      ...
      this.$body.find('> tr[data-index]').off('hover').hover(function () {
        var index = $(this).data('index');
        that.$rightfixedBody.find('tr[data-index="' + index + '"]').addClass(
            'hover');
      }, function () {
        var index = $(this).data('index');
        that.$rightfixedBody.find('tr[data-index="' + index + '"]').removeClass(
            'hover');
      });
      this.$rightfixedBody.find('tr[data-index]').off('hover').hover(
          function () {
            var index = $(this).data('index');
            that.$body.find('tr[data-index="' + index + '"]').addClass('hover');
          }, function () {
