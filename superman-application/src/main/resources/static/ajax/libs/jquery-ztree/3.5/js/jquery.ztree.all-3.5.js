  ...
  var settings = {}, roots = {}, caches = {},
      ...
      _eventProxy = function (event) {
        ...
        if (tools.eqs(event.type, "mousedown")) {
          ...
          if (tools.eqs(target.tagName, "span") && target.getAttribute(
            ...
            tId = ($(target).parent("li").get(0) || $(target).parentsUntil(
                "li").parent().get(0)).id;
            ...
            if (tmp) {
              tId = ($(tmp).parent("li").get(0) || $(tmp).parentsUntil(
                  "li").parent().get(0)).id;
      ...
      _initNode = function (setting, level, n, parentNode, isFirstNode,
        ...
        if (n[childKey] && n[childKey].length > 0) {
          ...
          if (typeof n.isParent == "string") {
            n.isParent = tools.eqs(n.isParent,
                "true");
          }
      ...
      data = {
        ...
        addNodesData: function (setting, parentNode, nodes) {
          ...
          if (parentNode[childKey].length > 0) {
            parentNode[childKey][parentNode[childKey].length
            - 1].isLastNode = false;
        ...
        getNodesByParam: function (setting, nodes, key, value) {
          ...
          for (var i = 0, l = nodes.length; i < l; i++) {
            ...
            result = result.concat(
                data.getNodesByParam(setting, nodes[i][childKey], key, value));
          }
          return result;
        },
        getNodesByParamFuzzy: function (setting, nodes, key, value) {
          ...
          for (var i = 0, l = nodes.length; i < l; i++) {
            ...
            result = result.concat(
                data.getNodesByParamFuzzy(setting, nodes[i][childKey], key,
                    value));
          }
          return result;
        },
        getNodesByFilter: function (setting, nodes, filter, isSingle,
          ...
          for (var i = 0, l = nodes.length; i < l; i++) {
            ...
            var tmpResult = data.getNodesByFilter(setting, nodes[i][childKey],
        ...
        transformToArrayFormat: function (setting, nodes) {
          ...
          if (tools.isArray(nodes)) {
            for (var i = 0, l = nodes.length; i < l; i++) {
              ...
              if (nodes[i][childKey]) {
                r = r.concat(
                    data.transformToArrayFormat(setting, nodes[i][childKey]));
              }
            }
          } else {
            r.push(nodes);
            if (nodes[childKey]) {
              r = r.concat(
                  data.transformToArrayFormat(setting, nodes[childKey]));
            }
          }
          return r;
        },
        transformTozTreeFormat: function (setting, sNodes) {
          ...
          if (tools.isArray(sNodes)) {
            ...
            for (i = 0, l = sNodes.length; i < l; i++) {
              if (tmpMap[sNodes[i][parentKey]] && sNodes[i][key]
                  != sNodes[i][parentKey]) {
        ...
        onClickNode: function (event, node) {
          var setting = data.getSetting(event.data.treeId),
              clickFlag = ((setting.view.autoCancelSelected && event.ctrlKey)
                  && data.isSelectedNode(setting, node)) ? 0
                  : (setting.view.autoCancelSelected && event.ctrlKey
                      && setting.view.selectedMulti) ? 2 : 1;
      ...
      tools = {
        ...
        canAsync: function (setting, node) {
          var childKey = setting.data.key.children;
          return (setting.async.enable && node && node.isParent && !(node.zAsync
        ...
        clone: function (obj) {
          ...
          for (var i in obj) {
            o[i] = (obj[i] instanceof Date) ? new Date(obj[i].getTime())
                : (typeof obj[i] === "object" ? arguments.callee(obj[i])
                    : obj[i]);
          }
          return o;
        },
      ...
      view = {
          ...
          for (var i = 0, l = nodes.length; i < l; i++) {
            ...
            if (initFlag) {
              var tmpPNode = (parentNode) ? parentNode : data.getRoot(setting),
                  ...
                  isFirstNode = ((tmpPChild.length == nodes.length) && (i
                      == 0)),
            ...
            if (node[childKey] && node[childKey].length > 0) {
              ...
              childHtml = view.appendNodes(setting, level + 1, node[childKey],
                  node, initFlag, openFlag && node.open);
            }
            if (openFlag) {
        ...
        appendParentULDom: function (setting, node) {
          var html = [],
              ...
              childKey = setting.data.key.children,
              childHtml = view.appendNodes(setting, node.level + 1,
                  node[childKey], node, false, true);
          view.makeUlHtml(setting, node, html, childHtml.join(''));
        ...
        cancelPreSelectedNode: function (setting, node) {
          ...
          for (var i = 0, j = list.length - 1; j >= i; j--) {
            if (!node || node === list[j]) {
              $("#" + list[j].tId + consts.id.A).removeClass(
        ...
        makeNodeIcoStyle: function (setting, node) {
          ...
          if (!node.isAjaxing) {
            var icon = (node.isParent && node.iconOpen && node.iconClose)
                ? (node.open ? node.iconOpen : node.iconClose) : node.icon;
            if (icon) {
            ...
            if (parentNode == root) {
              if (parentNode[childKey].length == 1) {
                ...
                var tmp_first_switchObj = $(
                    "#" + parentNode[childKey][0].tId + consts.id.SWITCH);
      ...
      var zTreeTools = {
        ...
        expandNode: function (node, expandFlag, sonSign, focus, callbackFlag) {
          ...
          if (expandFlag && node.parentTId) {
            view.expandCollapseParentNode(this.setting, node.getParentNode(),
                expandFlag, false);
          }
          ...
          if (sonSign) {
            view.expandCollapseSonNode(this.setting, node, expandFlag, true,
                function () {
                  if (focus !== false) {
                    try {
                      $("#" + node.tId).focus().blur();
        ...
        getNodeByParam: function (key, value, parentNode) {
          ...
          return data.getNodeByParam(this.setting,
              parentNode ? parentNode[this.setting.data.key.children]
                  : data.getNodes(this.setting), key, value);
        },
        ...
        getNodesByParam: function (key, value, parentNode) {
          ...
          return data.getNodesByParam(this.setting,
              parentNode ? parentNode[this.setting.data.key.children]
                  : data.getNodes(this.setting), key, value);
        },
        getNodesByParamFuzzy: function (key, value, parentNode) {
          ...
          return data.getNodesByParamFuzzy(this.setting,
              parentNode ? parentNode[this.setting.data.key.children]
                  : data.getNodes(this.setting), key, value);
        },
        getNodesByFilter: function (filter, isSingle, parentNode, invokeParam) {
          ...
          return data.getNodesByFilter(this.setting,
              parentNode ? parentNode[this.setting.data.key.children]
                  : data.getNodes(this.setting), filter, isSingle, invokeParam);
        },
        getNodeIndex: function (node) {
          ...
          var childKey = setting.data.key.children,
              parentNode = (node.parentTId) ? node.getParentNode()
        ...
        reAsyncChildNodes: function (parentNode, reloadType, isSilent) {
          ...
          if (reloadType == "refresh") {
            ...
            for (var i = 0,
                l = parentNode[childKey] ? parentNode[childKey].length : 0;
                i < l; i++) {
  ...
  var _consts = {
      ...
      _zTreeTools = function (setting, zTreeTools) {
        ...
        zTreeTools.getChangeCheckedNodes = function () {
          var childKey = this.setting.data.key.children;
          return data.getTreeChangeCheckedNodes(this.setting,
              data.getRoot(setting)[childKey]);
        }
      ...
      _data = {
          ...
          var checkedKey = setting.data.key.checked,
              r = {
                ...
                == consts.radio.STYLE ? (node.check_Child_State === 2)
                    : (node[checkedKey] ? (node.check_Child_State > -1
        ...
        getTreeCheckedNodes: function (setting, nodes, checked, results) {
          ...
          var childKey = setting.data.key.children,
              ...
              onlyOne = (checked && setting.check.chkStyle == consts.radio.STYLE
                  && setting.check.radioType == consts.radio.TYPE_ALL);
          results = !results ? [] : results;
          for (var i = 0, l = nodes.length; i < l; i++) {
            if (nodes[i].nocheck !== true && nodes[i].chkDisabled !== true
      ...
      _view = {
        checkNodeRelation: function (setting, node) {
          ...
          if (setting.check.chkStyle == r.STYLE) {
              ...
              if (setting.check.radioType == r.TYPE_ALL) {
                for (i = checkedList.length - 1; i >= 0; i--) {
                  ...
                  view.setChkClass(setting,
                      $("#" + pNode.tId + consts.id.CHECK), pNode);
            ...
            if (node[checkedKey] && (!node[childKey] || node[childKey].length
                == 0 || setting.check.chkboxType.Y.indexOf("s") > -1)) {
              view.setSonNodeCheckBox(setting, node, true);
            }
            if (!node[checkedKey] && (!node[childKey] || node[childKey].length
                == 0 || setting.check.chkboxType.N.indexOf("s") > -1)) {
              view.setSonNodeCheckBox(setting, node, false);
            }
            if (node[checkedKey] && setting.check.chkboxType.Y.indexOf("p")
                > -1) {
              view.setParentNodeCheckBox(setting, node, true);
            }
            if (!node[checkedKey] && setting.check.chkboxType.N.indexOf("p")
                > -1) {
        ...
        makeChkClass: function (setting, node) {
          ...
          if (node.chkDisabled === true) {
            ...
            fullStyle = node[checkedKey] ? ((node.check_Child_State === 2
                || node.check_Child_State === -1) ? c.FULL : c.PART)
                : ((node.check_Child_State < 1) ? c.FULL : c.PART);
          }
          var chkName = setting.check.chkStyle + "_" + (node[checkedKey]
              ? c.TRUE : c.FALSE) + "_" + fullStyle;
          ...
          if (node.parentTId) {
            ...
            if (!value) {
              ...
              for (var i = 0, l = pNodes.length; i < l; i++) {
                if ((pNodes[i].nocheck !== true && pNodes[i].chkDisabled
                    !== true && pNodes[i][checkedKey])
                    || ((pNodes[i].nocheck === true || pNodes[i].chkDisabled
                        === true) && pNodes[i].check_Child_State > 0)) {
                  pSign = false;
                  break;
                }
              }
        ...
        setSonNodeCheckBox: function (setting, node, value, srcNode) {
          ...
          if (node[childKey]) {
            for (var i = 0, l = node[childKey].length;
                i < l && node.chkDisabled !== true; i++) {
              var sNode = node[childKey][i];
          ...
          if (node != data.getRoot(setting) && node.chkDisabled !== true) {
            ...
            if (node.nocheck !== true && node.chkDisabled !== true) {
              node[checkedKey] = value;
              if (!hasDisable) {
                node.check_Child_State = (node[childKey]
                    && node[childKey].length > 0) ? (value ? 2 : 0) : -1;
              }
            } else {
  ...
  var _consts = {
      ...
      _zTreeTools = function (setting, zTreeTools) {
        ...
        zTreeTools.editName = function (node) {
          if (!node || !node.tId || node !== data.getNodeCache(setting,
              node.tId)) {
            return;
          }
          if (node.parentTId) {
            view.expandCollapseParentNode(setting,
        ...
        zTreeTools.moveNode = function (targetNode, node, moveType, isSilent) {
          ...
          if (targetNode && !targetNode.isParent && setting.data.keep.leaf
            ...
            return null;
              ...
              && moveType == consts.move.TYPE_INNER) || $("#" + node.tId).find(
              "#" + targetNode.tId).length > 0)) {
            return null;
          } else if (!targetNode) {
            targetNode = null;
          }
          ...
          if (tools.canAsync(setting, targetNode) && moveType
        ...
        onMousedownNode: function (eventMouseDown, _node) {
          ...
          function _docMouseMove(event) {
            ...
            if (root.dragFlag == 0) {
              ...
              if (nodes.length > 1) {
                var pNodes = nodes[0].parentTId
                    ? nodes[0].getParentNode()[childKey] : data.getNodes(
                        setting);
                tmpNodes = [];
            ...
            if (root.dragFlag == 1) {
              ...
              if (event.target.id && targetSetting.treeObj.find(
                ...
                if (canMove) {
                  if (event.target.id &&
                      (event.target.id == (targetObj.id + consts.id.A) || $(
                          event.target).parents(
                          "#" + targetObj.id + consts.id.A).length > 0)) {
                    tmpTarget = $(targetObj);
              ...
              tmpNode = nodes[0];
              if (isTreeInner && (event.target.id == targetSetting.treeId || $(
                  event.target).parents("#" + targetSetting.treeId).length
                  > 0)) {
              ...
              if (tmpTarget && tmpTarget.attr("id") != targetSetting.treeId) {
                var tmpTargetNode = tmpTargetNodeId == null ? null
                    ...
                    isCopy = (event.ctrlKey && setting.edit.drag.isMove
                        && setting.edit.drag.isCopy)
                        || (!setting.edit.drag.isMove
                            && setting.edit.drag.isCopy),
                    ...
                    canInner = (isCopy || !isInner)
                        && !(targetSetting.data.keep.leaf
                            && !tmpTargetNode.isParent) && tools.apply(
                ...
                if (!canPrev && !canNext && !canInner) {
                  ...
                  if ((prevPercent == 1 || dY_percent <= prevPercent
                    ...
                    dX = 1 - tmpArrow.width();
                    dY = (tmpNextA == null || (tmpTargetNode.isParent
                        && tmpTargetNode.open)) ? (tmpTop + tmpTargetA.height()
                        - tmpArrow.height() / 2) : (tmpNextA.offset().top
                        - tmpArrow.height() / 2);
                  ...
                  if (tmpTargetNode && tmpTargetNode.isParent && moveType
                      == consts.move.TYPE_INNER) {
                    var startTimer = true;
          ...
          function _docMouseUp(event) {
            ...
            for (i = 0, l = nodes.length; i < l; i++) {
              tmpNode = nodes[i];
              if (tmpNode.isParent && root.dragNodeShowBefore[tmpNode.tId]
                  && !tmpNode.open) {
                view.expandCollapseNode(setting, tmpNode, !tmpNode.open);
            ...
            var isCopy = (event.ctrlKey && setting.edit.drag.isMove
                  ...
                  if (isCopy && moveType == consts.move.TYPE_INNER) {
                    ...
                    if (isCopy) {
                      view.addNodes(targetSetting,
                          dragTargetNode.getParentNode(), newNodes);
                    }
      ...
      _view = {
        addEditBtn: function (setting, node) {
          if (node.editNameFlag || $("#" + node.tId + consts.id.EDIT).length
              > 0) {
            return;
          }
        ...
        addRemoveBtn: function (setting, node) {
          if (node.editNameFlag || $("#" + node.tId + consts.id.REMOVE).length
              > 0) {
            return;
          }
        ...
        cancelCurEditNode: function (setting, forceName) {
          ...
          if (node) {
            ...
            if (!forceName && tools.apply(setting.callback.beforeRename,
        ...
        editNode: function (setting, node) {
          ...
          $("#" + node.tId + consts.id.A).addClass(
              consts.node.CURSELECTED_EDIT);
        ...
        moveNode: function (setting, targetNode, node, moveType, animateFlag,
          ...
          if (!nodeDom.get(0)) {
            nodeDom = view.appendNodes(setting, node.level, [node], null, false,
                true).join('');
          ...
          if (moveType != consts.move.TYPE_INNER) {
            for (i = 0, l = targetParentNode[childKey].length; i < l; i++) {
              if (targetParentNode[childKey][i].tId
                  == targetNode.tId) {
                tmpTargetIndex = i;
              }
            }
...
(function ($) {
  ...
  var _initNode = function (setting, level, n, parentNode, isFirstNode,
      isLastNode, openFlag) {
        if (typeof n.isHidden == "string") {
          n.isHidden = tools.eqs(n.isHidden,
              "true");
        }
      ...
      _view = {
        ...
        makeDOMNodeMainBefore: function (html, setting, node) {
          html.push("<li ", (node.isHidden ? "style='display:none;' " : ""),
              "id='", node.tId, "' class='", consts.className.LEVEL, node.level,
