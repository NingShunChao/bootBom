...
(function ($) {
  function init(plot) // this is the "body" of the plugin
  {
    ...
    function setupPie() {
      ...
      maxRadius = Math.min(canvas.width,
          (canvas.height / options.series.pie.tilt)) / 2;
    ...
    function draw(plot, newCtx) {
      ...
      function drawShadow() {
        ...
        if (radius >= (canvas.width / 2) - shadowLeft || radius
            * options.series.pie.tilt >= (canvas.height / 2) - shadowTop
            || radius <= edge) {
          return;
      ...
      function drawPie() {
        ...
        function drawLabels() {
          ...
          function drawLabel(slice, startAngle, index) {
            ...
            // format label text
            var lf = options.legend.labelFormatter, text,
            ...
            var html = '<span class="pieLabel" id="pieLabel' + index
            ...
            if (0 - labelTop > 0 || 0 - labelLeft > 0 || canvas.height
                - (labelTop + label.height()) < 0 || canvas.width - (labelLeft
                    + label.width()) < 0) {
              redraw = true;
            }
            ...
            if (options.series.pie.label.background.opacity != 0) {
              ...
              $('<div class="pieLabelBackground" style="position:absolute;width:'
                  + label.width() + 'px;height:' + label.height() + 'px;' + pos
                  + 'background-color:' + c + ';"> </div>').insertBefore(
                  label).css('opacity',
    ...
    function drawDonutHole(layer) {
      ...
      if (options.series.pie.innerRadius > 0) {
        // subtract the center
        layer.save();
        innerRadius = options.series.pie.innerRadius > 1
            ? options.series.pie.innerRadius : maxRadius
    ...
    function isPointInPoly(poly, pt) {
      for (var c = false, i = -1, l = poly.length, j = l - 1; ++i < l; j = i) {
        ((poly[i][1] <= pt[1] && pt[1] < poly[j][1]) || (poly[j][1] <= pt[1]
            && pt[1] < poly[i][1]))
    ...
    function findNearbySlice(mouseX, mouseY) {
      var slices = plot.getData(),
          options = plot.getOptions(),
          radius = options.series.pie.radius > 1 ? options.series.pie.radius
      ...
      for (var i = 0; i < slices.length; ++i) {
        ...
        if (s.pie.show) {
          ...
          if (ctx.isPointInPath) {
            ...
            arrPoly = [[0, 0], [p1X, p1Y], [p2X, p2Y], [p3X, p3Y], [p4X, p4Y],
              [p5X, p5Y]];
            arrPoint = [x, y];
    ...
    function drawOverlay(plot, octx) {
      ...
      var radius = options.series.pie.radius > 1 ? options.series.pie.radius
          : maxRadius * options.series.pie.radius;
      ...
      function drawHighlight(series) {
        ...
        octx.fillStyle = "rgba(255, 255, 255, "
            + options.series.pie.highlight.opacity + ")"; // this is temporary until we have access to parseColor
        ...
        octx.arc(0, 0, radius, series.startAngle,
            series.startAngle + series.angle, false);
        octx.closePath();
        octx.fill();
      }
        ...
        label: {
          ...
          formatter: function (label, slice) {
            return '<div style="font-size:x-small;text-align:center;padding:2px;color:'
                + slice.color + ';">' + label + '<br/>' + Math.round(
                    slice.percent) + '%</div>';
          },	// formatter function
