...
(function ($) {
  ...
  $.fn.bootstrapTreeTable = function (options, param) {
    ...
    var initToolbar = function () {
      ...
      if (options.showSearch) {
        var $searchBtn = $(
            '<button class="btn btn-default btn-outline" type="button" aria-label="search" title="搜索"><i class="glyphicon glyphicon-search"></i></button>');
      ...
      if (options.showRefresh) {
        var $refreshBtn = $(
            '<button class="btn btn-default btn-outline" type="button" aria-label="refresh" title="刷新"><i class="glyphicon glyphicon-repeat"></i></button>');
      ...
      if (options.showColumns) {
        var $columns_div = $(
            '<div class="btn-group pull-right" title="列"><button type="button" aria-label="columns" class="btn btn-default btn-outline dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="glyphicon glyphicon-list"></i> <span class="caret"></span></button></div>');
        var $columns_ul = $(
            '<ul class="dropdown-menu columns" role="menu"></ul>');
          ...
          if (column.field != 'selectItem') {
            ...
            if (typeof column.visible == "undefined" || column.visible
              ...
              _li = $(
                  ...
                  + column.title + '</label></li>');
            } else {
              _li = $(
                  '<li role="menuitem"><label><input type="checkbox" data-field="'
                  + column.field + '" value="' + column.field + '" > '
                  + column.title + '</label></li>');
    ...
    var initServer = function (parms) {
      ...
      // 添加加载loading
      var $loading = '<tr><td colspan="' + options.columns.length
      ...
      if (options.url) {
        $.ajax({
          ...
          success: function (data, textStatus, jqXHR) {
            data = calculateObjectValue(options, options.responseHandler,
                [data], data);
            renderTable(data);
          },
          error: function (xhr, textStatus) {
            var _errorMsg = '<tr><td colspan="' + options.columns.length
                + '"><div style="display: block;text-align: center;">'
                + xhr.responseText + '</div></td></tr>'
    ...
    var renderTable = function (data) {
      ...
      if (!data || data.length <= 0) {
        var _empty = '<tr><td colspan="' + options.columns.length
    ...
    var autoTheadWidth = function (initFlag) {
      if (options.height > 0) {
        ...
        var $tbody = target.find("tbody");
        var borderWidth = parseInt(target.css("border-left-width")) + parseInt(
    ...
    var formatData = function (data) {
      ...
      $.each(data, function (index, item) {
        ...
        if (!item[options.parentCode] || (_root ? (item[options.parentCode]
            == options.rootIdValue) : _defaultRootFlag)) {
          rootFlag = true;
    ...
    var renderRow = function (item, isP, lv, row_id, p_id) {
      ...
      if (options.expandAll) {
        ...
        $tr.css("display", "table");
        _icon = (options.expandFirst) ? options.expanderExpandedClass
            : options.expanderCollapsedClass;
      } else if (lv == 2) {
      ...
      $.each(options.columns, function (index, column) {
        ...
        if (column.field == 'selectItem') {
          ...
          if (column.radio) {
            var _ipt = $('<input name="select_item" type="radio" value="'
                + item[options.code] + '"></input>');
            $td.append(_ipt);
          }
          if (column.checkbox) {
            var _ipt = $('<input name="select_item" type="checkbox" value="'
                + item[options.code] + '"></input>');
            $td.append(_ipt);
          }
          $tr.append($td);
        } else {
          var $td = $('<td name="' + column.field + '" class="' + column.field
              + '_cls"></td>');
          if (column.width) {
          ...
          if (column.formatter) {
            $td.html(
                column.formatter.call(this, getItemField(item, column.field),
                    item, index));
          } else {
            if (options.showTitle) {
          ...
          if (options.expandColumn == index) {
            if (!isP) {
              ...
              $td.prepend(
                  '<span class="treetable-expander ' + _icon + '"></span>')
            }
    ...
    var registerColumnClickEvent = function () {
      $(".bootstrap-tree-table .treetable-bars .columns label input").off(
    ...
    var registerExpanderEvent = function () {
      ...
      target.find("tbody").find("tr").find(".treetable-expander").click(
          function () {
            ...
            if (_isExpanded || _isCollapsed) {
              ...
              if (_isExpanded) {
                ...
                if (_ls && _ls.length > 0) {
                  $.each(_ls, function (index, item) {
                    // 父icon
                    var _p_icon = $("#" + $(item).attr("pid")).children().eq(
                        options.expandColumn).find(".treetable-expander");
    ...
    target.appendData = function (data) {
      ...
      $.each(data, function (i, item) {
        ...
        if (_p_data) {
          ...
          var _p_icon = $("#" + _p_data.row_id).children().eq(
              options.expandColumn).find(".treetable-expander");
          ...
          if (_data) {
            ...
            var _p_next = _p_data.row_id.substring(0, _p_data.row_id.length - 1)
                + (parseInt(_tmp_ls[_tmp_ls.length - 1]) + 1);
            // 画上
            $("#" + _p_next).before(tr);
          }
        } else {
    ...
    target.expandRow = function (id) {
      var _rowData = target.data_obj["id_" + id];
      var $row_expander = $("#" + _rowData.row_id).find(".treetable-expander");
      var _isCollapsed = $row_expander.hasClass(
          target.options.expanderCollapsedClass);
      if (_isCollapsed) {
        $row_expander.trigger("click");
      }
    }
    ...
    target.collapseRow = function (id) {
      var _rowData = target.data_obj["id_" + id];
      var $row_expander = $("#" + _rowData.row_id).find(".treetable-expander");
      var _isExpanded = $row_expander.hasClass(
          target.options.expanderExpandedClass);
      if (_isExpanded) {
        $row_expander.trigger("click");
      }
    }
    ...
    target.expandAll = function () {
      target.find("tbody").find("tr").find(".treetable-expander").each(
          function (i, n) {
    ...
    target.showColumn = function (field, flag) {
      ...
      if (flag && options.showColumns) {
        var $input = $(
            ".bootstrap-tree-table .treetable-bars .columns label").find(
            "input[value='" + field + "']")
    ...
    var calculateObjectValue = function (self, name, args, defaultValue) {
      ...
      if (!func && typeof name === 'string' && sprintf.apply(this,
          [name].concat(args))) {
  ...
  $.fn.bootstrapTreeTable.methods = {
    ...
    getSelections: function (target, data) {
      // 所有被选中的记录input
      var _ipt = target.find("tbody").find("tr").find(
