...
(function (factory) {
  ...
  function setOrGetDataId($input, val) {
    return val !== (void 0) ? $input.attr('data-id', val) : $input.attr(
        'data-id');
  }
  ...
  function setValue($input, keywords, options) {
    ...
    .trigger('onSetSelectValue', [keywords,
  ...
  function setBackground($input, options) {
    ...
    inputbg = $input.css('backgroundColor').replace(/ /g, '').split(',',
        3).join(',');
    // 自由输入的内容，设置背景色
  ...
  function unHoverAll($dropdownMenu, options) {
    $dropdownMenu.find('tr.' + options.listHoverCSS).removeClass(
  ...
  function showTip(tip, $input, $dropdownMenu, options) {
    $dropdownMenu.html(
        '<div style="padding:10px 5px 5px">' + tip + '</div>').show();
  ...
  function refreshDropMenu($input, data, options) {
    ...
    setTimeout(function () {
      ...
      if (
          ...
          +$dropdownMenu.css('minWidth').replace('px', '')
          < $dropdownMenu.width()
      ) {
        pdr = 18;
        mgb = 20;
    ...
    if (options.url) {
      ...
      if (options.searchingTip) {
        timer = setTimeout(function () {
          showTip(options.searchingTip, $input, $input.parent().find('ul'),
              options);
        }, 600);
      }
  ...
  var methods = {
    init: function (options) {
      ...
      if (isUndefined(options.showHeader) && options.effectiveFields
      ...
      if (!$('#' + BSSUGGEST).length) {
        $('head:eq(0)').append(
            '<style id="' + BSSUGGEST + '">.' + options.listHoverCSS + '{'
            + options.listHoverStyle + '}</style>');
      }
      ...
      return self.each(function () {
        ...
        $input.removeClass(DISABLED).prop(DISABLED, FALSE).attr('autocomplete',
            'off');
        // dropdown-menu 增加修饰
        ...
        $input.on('keydown', function (event) {
          ...
          if (~$.inArray(event.keyCode,
              [options.keyDown, options.keyUp, options.keyEnter])) {
        ...
        if ($iClear.length) {
          ...
          $parent.mouseenter(function () {
            if (!$input.prop(DISABLED)) {
              $iClear.css('right',
                  options.showBtn ? Math.max($input.next().width(), 33) + 2
                      : 12)
              .show();
            }
          }).mouseleave(function () {
    ...
    destroy: function () {
      return this.each(function () {
        ...
        .parent().find('.btn:eq(0)').off().show().attr('data-toggle',
            'dropdown').prop(DISABLED, FALSE) // .addClass(DISABLED);
